...
async def _make_room(client: AsyncClient) -> str:
    resp = await client.post(
        "/rooms", json={"name": ROOM, "created_by": SENDER}, headers=HEADERS,
    )
    assert resp.status_code == 200, resp.text
    room_id = resp.json()["id"]
    j = await client.post(
        f"/rooms/{room_id}/join", json={"participant": RECIPIENT}, headers=HEADERS,
    )
    ...
    while len(out) < expected:
        ...
        try:
            ...
            raise AssertionError(
                f"timeout draining SSE: {len(out)}/{expected}: {out!r}",
            ) from e
...
async def test_subscriber_disconnect_messages_replay_on_reconnect(
    ...
    1. bob registers SSE queue, sees N=3 messages live.
    2. bob's SSE queue is unregistered (= SIGTERM on his daemon).
    3. M=4 more messages fan out to his per-recipient deque (the durable
       outbox analogue) but no live SSE consumer.
    4. bob reconnects: registers a fresh SSE queue + drains backlog via
       long-poll (this is the outbox catch-up).
    5. Assert: bob saw N + M + 1 messages in send order.
    """
    ...
    # disconnect (long-poll ack=server) so the phase-3 backlog is the
    ...
    assert backlog_len == m, (
        f"expected {m} queued while disconnected, got {backlog_len}"
    )
...
@pytest.mark.skipif(
    ...
    reason="Audit ledger requires DATABASE_URL; in-memory mode constructs "
    "no AuditService (see quorus/relay.py::_init_services).",
)
async def test_audit_chain_intact_after_subscriber_disconnect(
    client: AsyncClient,
):
    """When Postgres is wired, the audit chain records lifecycle events
    AND verifies its hash chain even when an SSE subscriber disconnects
    mid-stream. Asserts on the real ``AuditEvent`` enum names.
    """
