#!/usr/bin/env python3
"""Quorus Watcher — SSE listener that writes to inbox file + macOS notifications.

Usage:
    python -m quorus.integrations.watcher --name agent-1 --relay http://localhost:8080 --secret s

Writes new messages to /tmp/quorus-{name}-inbox.txt (one JSON per line).
On macOS, also shows a system notification for each message.

Agents can read the inbox file in their CLAUDE.md:
    "Read /tmp/quorus-{name}-inbox.txt before every task."
"""
...
def watch(name: str, relay_url: str, secret: str, inbox_path: str | None = None):
    ...
    if inbox_path is None:
        inbox_path = f"/tmp/quorus-{name}-inbox.txt"

    inbox = Path(inbox_path)
    inbox.touch()

    print(f"Quorus watcher for '{name}'")
...
def main():
    parser = argparse.ArgumentParser(description="Quorus message watcher")
    ...
    parser.add_argument("--inbox", help="Inbox file path (default: /tmp/quorus-{name}-inbox.txt)")
