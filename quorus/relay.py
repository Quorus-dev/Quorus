"""Quorus Relay — FastAPI app setup, lifespan, middleware, and router wiring.
...
logger = structlog.get_logger("quorus.relay")
...
async def _run_migrations():
    ...
    try:
        ...
        alembic_cfg = Config()
        alembic_cfg.set_main_option("script_location", "quorus/migrations")
...
def _init_services(app_instance, redis_conn=None):
    ...
    if redis_conn is not None:
        from quorus.backends.redis_backends import RedisBackends

        backends = RedisBackends.create(
            redis_conn, max_room_history=max_room_history
        )
    else:
        from quorus.backends.memory import InMemoryBackends
...
async def lifespan(app):
    ...
    redis_url = os.environ.get("REDIS_URL", "")
    if redis_url:
        from quorus.backends.redis_client import init_redis
    ...
    if redis_url:
        from quorus.backends.redis_client import close_redis
        await close_redis()

    # Close Postgres if it was initialized
    if DATABASE_URL:
        from quorus.storage.postgres import close_engine
        await close_engine()
...
from quorus.admin.routes import router as admin_router  # noqa: E402
